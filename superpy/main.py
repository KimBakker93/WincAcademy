...
def main():
    ...
    parser.add_argument('--report-type', type = str, help = 'Type of report, must be either revenue, profit or inventory.')
    parser.add_argument('--product-name', type = str, help = 'Name of a product the supermarket offers.')
    parser.add_argument('--bought-id', type = int, help = 'Id of the sold product as it is declared in the bought.csv file.')
    parser.add_argument('--price', type = float, help = 'The amount of money per product the supermarket paid for a product or sold it for.')
    parser.add_argument('--expiration-date', type = str, help = 'The date on which the product the supermarket has in stock expires. The date shows the year first, than the month and lastly the day.')
    parser.add_argument('--advance-time', default = 0, type = int, help = 'A tool which you can use to fastworward the time to two days later.')
    parser.add_argument('--date', type = str, help = 'Manner to specify a specific moment in time.')
    parser.add_argument('--yesterday', action = 'store_true', help = 'Manner to specify a specific moment in time, only used for reports.')
    ...
    if args.type == 'report':
        ...
        if args.report_type == 'inventory':
            report.report_inventory(period,check_date)    
